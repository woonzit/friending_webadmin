...
/**
 * Core's production-generated wire corpus (`tests/fixtures/persona_screens_wire/`),
 * regenerated by T-588 at content commit `8e9a82d…` and deployed in `61ce859`,
 * copied byte-identically. Every body here came out of the production resolver,
 * the production admin service and the production encoders, so the Webadmin
 * decoders are verified against what Core actually publishes rather than
 * against a reading of the contract.
 *
 * T-588 removed the old corpus's vacuity: every served compiled-copy string is
 * now conspicuous operator text and differs from `compiled_reference`. The EN
 * and partial-HU documents also carry the complete T-593 PRE external-link
 * pairs, so this corpus detects a reader that silently substitutes compiled
 * copy and exercises the additive D-089 slot in both languages.
 */
const FIXTURE_DIRECTORY = new URL("./fixtures/persona_screens_wire/", import.meta.url);
const FIXTURE_CONTRACT_VERSION = 1;
const FIXTURE_SOURCE_COMMIT = "8e9a82dce491ab42e26d9339c805868c360ad340";
const FIXTURE_MANIFEST_SHA256 = "2fddf95b27586b2872055b6d0bbbcf9138954c1e7ca5b62acf0a46a434b49d68";
...
const FIXTURE_GENERATOR_SHA256 = "42625f805e2ee4e06c80cc65ce4e31aafc2ac88bae87416ff648dbbdc03f29e8";
...
test("the console read decodes with its revision, its stored document and the complete reference", async () => {
  ...
  // Mid-translation: two Hungarian strings plus the complete PRE link.
  assert.deepEqual(clean.value.copy_default.hu, {
    pre: {
      headline: "Operátori HU: személyazonosság-ellenőrzés",
      external_link: {
        label: "Operátori HU: ellenőrzési súgó",
        url: "https://operator.example/hu/persona-sugo",
      },
    },
    failed: { cta: "Operátori HU: újra" },
  });
...
test("the console reader keeps all conspicuous operator copy instead of silently falling back", async () => {
  const clean = decodePersonaScreensConsoleResponse(await fixture("console-read-clean.json"));
  assert.ok(clean.ok, "console-read-clean");

  const fallbacks: string[] = [];
  const inconspicuous: string[] = [];
  let storedStrings = 0;
  for (const language of PERSONA_SCREEN_LANGUAGES) {
    for (const screen of PERSONA_SCREEN_KEYS) {
      for (const slot of PERSONA_SCREEN_SLOTS) {
        const stored = clean.value.copy_default[language]?.[screen]?.[slot];
        if (typeof stored !== "string") continue;
        const path = `${language}.${screen}.${slot}`;
        storedStrings += 1;
        if (stored === clean.value.compiled_reference[language][screen][slot]) fallbacks.push(path);
        const prefix = language === "en" ? "Operator EN:" : "Operátori HU:";
        if (!stored.startsWith(prefix)) inconspicuous.push(path);
      }
    }
  }
  assert.deepEqual(fallbacks, [], `reader silently fell back at ${fallbacks.join(", ")}`);
  assert.deepEqual(inconspicuous, [], `fixture copy is not conspicuous at ${inconspicuous.join(", ")}`);
  assert.equal(storedStrings, 11, "nine English and two Hungarian operator strings are exercised");
  assert.deepEqual(clean.value.copy_default.hu?.pre?.external_link, {
    label: "Operátori HU: ellenőrzési súgó",
    url: "https://operator.example/hu/persona-sugo",
  });
});
...
test("the console's published projection equals Core's published block on every appconfig fixture", async () => {
  ...
  const cases: ReadonlyArray<[string, unknown, PersonaScreenLanguage]> = [
    ...
    ["appconfig-slot-degrades-alone.json", {
      en: {
        pre: {
          headline: "Operator EN: degradation headline",
          subtitle: "x".repeat(321),
          cta: "Operator EN: degrade",
        },
      },
    }, "en"],
