...
test("all authenticated page routes have one closed contextual help entry", async () => {
  ...
  assert.equal(actualRoutes.length, 36, "the current screen census changed; review every new or removed screen");
...
test("every inventoried functional section has detailed English and Hungarian help", async () => {
  ...
  assert.equal(totalSections, 191, "review the functional-section census when the UI changes");
