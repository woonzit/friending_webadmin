...
function SelectField({
  label, value, effective, effectiveLabel, options, disabled, inheritValue, onChange,
}: {
  label: string; value: string; effective: string; options: readonly { value: string; label: string }[];
  effectiveLabel: string; disabled: boolean; inheritValue: string; onChange: (value: string) => void;
}) {
  return (
    <label className="field">
      <span>{label}</span>
      ...
      <small className="appearance-landing-effective-value">{value === "" ? effectiveLabel : effective}</small>
    </label>
  );
}

function ColorField({
  label, value, effective, disabled, inheritLabel, effectiveLabel, onChange,
}: {
  label: string; value: string; effective: string; disabled: boolean; inheritLabel: string;
  effectiveLabel: string; onChange: (value: string) => void;
}) {
  ...
  return (
    <label className="field appearance-landing-color-field">
      ...
      <span className="appearance-landing-color-row">
        <input value={value} maxLength={7} disabled={disabled} spellCheck={false} placeholder={effective} onChange={(event) => onChange(event.target.value.toUpperCase())} />
...
function RangeField({
  label, value, effective, minimum, maximum, step, disabled, inheritLabel, effectiveLabel, onChange,
}: {
  label: string; value: string; effective: string; minimum: number; maximum: number; step: number;
  disabled: boolean; inheritLabel: string; effectiveLabel: string; onChange: (value: string) => void;
}) {
  ...
  return (
    <label className="field appearance-landing-range-field">
      ...
      <span className="appearance-landing-range-row">
        <input type="range" min={minimum} max={maximum} step={step} value={rangeValue} disabled={disabled} onChange={(event) => onChange(canonical(Number(event.target.value)))} />
        <input
          type="number"
          min={minimum}
          max={maximum}
...
export default function AppearanceLandingComposer({
  ...
  const contentByLanguage = useMemo(
    () => ({
      en: resolveAppearanceLanding([draftWire, parent], activeDefaults, "en"),
      hu: resolveAppearanceLanding([draftWire, parent], activeDefaults, "hu"),
    }),
    [draftWire, parent, activeDefaults],
  );
  ...
  const metaEffective = (key: AppearanceLandingKey, value: string) => {
    ...
    if (parentState.kind !== "ready") return t("effectiveValue", { value: displayValue });
    let sourceKey = key;
  ...
  async function compareSaved() {
    if (!persistedRule || comparison.kind === "loading") return;
    ...
    <div className="appearance-landing-composer field-full">
      <div className="appearance-landing-composer-fields">
        <ComposerSection title={t("background.title")} copy={t("background.copy")}>
          <SelectField
            ...
            value={rule.landing.background_type}
            effective={effective.background_type}
            ...
            onChange={(value) => {
              if (value !== rule.landing.background_type) patchMany({ background_type: value, background_url: "" });
            }}
          />
          {effective.background_type === "video" ? (
            <VideoUploadField
              ...
              onChange={(url) => patchMany({ background_url: url, background_type: url === "" ? "" : "video" })}
            />
          ) : (
            <ImageUploadField
              className="field-full"
              ...
              hint={uploadHint("background_url", effective.background_url)}
              onBusyChange={onBusyChange}
              onChange={(url) => patchMany({ background_url: url, background_type: url === "" ? "" : "image" })}
            />
          )}
          ...
          <ColorField label={t("background.overlayColor")} value={rule.landing.overlay_color} effective={effective.overlay_color} disabled={disabled} inheritLabel={inheritLabel} effectiveLabel={metaEffective("overlay_color", effective.overlay_color)} onChange={(value) => patch("overlay_color", value)} />
          <RangeField label={t("background.overlayAlpha")} value={rule.landing.overlay_alpha} effective={effective.overlay_alpha} minimum={0} maximum={1} step={0.01} disabled={disabled} inheritLabel={inheritLabel} effectiveLabel={metaEffective("overlay_alpha", effective.overlay_alpha)} onChange={(value) => patch("overlay_alpha", value)} />
        </ComposerSection>
        ...
        <ComposerSection title={t("title.title")} copy={t("title.copy")}>
          ...
          <SelectField label={t("title.font")} value={rule.landing.title_font} effective={effective.title_font} effectiveLabel={metaEffective("title_font", effective.title_font)} disabled={disabled} inheritValue={inheritValue(t(`font.${effective.title_font as AppearanceLandingFont}`))} options={fontOptions} onChange={(value) => patch("title_font", value)} />
          <RangeField label={t("title.size")} value={rule.landing.title_size} effective={effective.title_size} minimum={12} maximum={72} step={1} disabled={disabled} inheritLabel={inheritLabel} effectiveLabel={metaEffective("title_size", effective.title_size)} onChange={(value) => patch("title_size", value)} />
          <ColorField label={t("title.color")} value={rule.landing.title_color} effective={effective.title_color} disabled={disabled} inheritLabel={inheritLabel} effectiveLabel={metaEffective("title_color", effective.title_color)} onChange={(value) => patch("title_color", value)} />
          <SelectField label={t("title.align")} value={rule.landing.title_align} effective={effective.title_align} effectiveLabel={metaEffective("title_align", effective.title_align)} disabled={disabled} inheritValue={inheritValue(t(`align.${effective.title_align}`))} options={alignOptions} onChange={(value) => patch("title_align", value)} />
        ...
        <ComposerSection title={t("buttons.title")} copy={t("buttons.copy")}>
          ...
          <ColorField label={t("buttons.background")} value={rule.landing.button_phone_bg} effective={effective.button_phone_bg} disabled={disabled} inheritLabel={inheritLabel} effectiveLabel={metaEffective("button_phone_bg", effective.button_phone_bg)} onChange={(value) => patch("button_phone_bg", value)} />
          <ColorField label={t("buttons.textColor")} value={rule.landing.button_phone_text_color} effective={effective.button_phone_text_color} disabled={disabled} inheritLabel={inheritLabel} effectiveLabel={metaEffective("button_phone_text_color", effective.button_phone_text_color)} onChange={(value) => patch("button_phone_text_color", value)} />
          <SelectField label={t("buttons.font")} value={rule.landing.button_phone_font} effective={effective.button_phone_font} effectiveLabel={metaEffective("button_phone_font", effective.button_phone_font)} disabled={disabled} inheritValue={inheritValue(t(`font.${effective.button_phone_font as AppearanceLandingFont}`))} options={fontOptions} onChange={(value) => patch("button_phone_font", value)} />
          <RangeField label={t("buttons.size")} value={rule.landing.button_phone_size} effective={effective.button_phone_size} minimum={12} maximum={24} step={1} disabled={disabled} inheritLabel={inheritLabel} effectiveLabel={metaEffective("button_phone_size", effective.button_phone_size)} onChange={(value) => patch("button_phone_size", value)} />
          ...
          <ColorField label={t("buttons.background")} value={rule.landing.button_email_bg} effective={effective.button_email_bg} disabled={disabled} inheritLabel={inheritLabel} effectiveLabel={metaEffective("button_email_bg", effective.button_email_bg)} onChange={(value) => patch("button_email_bg", value)} />
          <ColorField label={t("buttons.textColor")} value={rule.landing.button_email_text_color} effective={effective.button_email_text_color} disabled={disabled} inheritLabel={inheritLabel} effectiveLabel={metaEffective("button_email_text_color", effective.button_email_text_color)} onChange={(value) => patch("button_email_text_color", value)} />
          ...
          <RangeField label={t("buttons.size")} value={rule.landing.button_email_size} effective={effective.button_email_size} minimum={12} maximum={24} step={1} disabled={disabled} inheritLabel={inheritLabel} effectiveLabel={metaEffective("button_email_size", effective.button_email_size)} onChange={(value) => patch("button_email_size", value)} />
          <SelectField
        ...
        <ComposerSection title={t("footer.title")} copy={t("footer.copy")}>
          ...
          <ColorField label={t("footer.background")} value={rule.landing.footer_bg_color} effective={effective.footer_bg_color} disabled={disabled} inheritLabel={inheritLabel} effectiveLabel={metaEffective("footer_bg_color", effective.footer_bg_color)} onChange={(value) => patch("footer_bg_color", value)} />
          <RangeField label={t("footer.alpha")} value={rule.landing.footer_bg_alpha} effective={effective.footer_bg_alpha} minimum={0} maximum={1} step={0.01} disabled={disabled} inheritLabel={inheritLabel} effectiveLabel={metaEffective("footer_bg_alpha", effective.footer_bg_alpha)} onChange={(value) => patch("footer_bg_alpha", value)} />
          <ColorField label={t("footer.textColor")} value={rule.landing.footer_text_color} effective={effective.footer_text_color} disabled={disabled} inheritLabel={inheritLabel} effectiveLabel={metaEffective("footer_text_color", effective.footer_text_color)} onChange={(value) => patch("footer_text_color", value)} />
          <SelectField label={t("footer.font")} value={rule.landing.footer_font} effective={effective.footer_font} effectiveLabel={metaEffective("footer_font", effective.footer_font)} disabled={disabled} inheritValue={inheritValue(t(`font.${effective.footer_font as AppearanceLandingFont}`))} options={fontOptions} onChange={(value) => patch("footer_font", value)} />
          <RangeField label={t("footer.size")} value={rule.landing.footer_size} effective={effective.footer_size} minimum={10} maximum={18} step={1} disabled={disabled} inheritLabel={inheritLabel} effectiveLabel={metaEffective("footer_size", effective.footer_size)} onChange={(value) => patch("footer_size", value)} />
        </ComposerSection>
        ...
        <ComposerSection title={t("qr.title")} copy={t("qr.copy")}>
          ...
          />
          <ColorField label={t("qr.background")} value={rule.landing.qr_bg_color} effective={effective.qr_bg_color} disabled={disabled} inheritLabel={inheritLabel} effectiveLabel={metaEffective("qr_bg_color", effective.qr_bg_color)} onChange={(value) => patch("qr_bg_color", value)} />
          <ColorField label={t("qr.icon")} value={rule.landing.qr_icon_color} effective={effective.qr_icon_color} disabled={disabled} inheritLabel={inheritLabel} effectiveLabel={metaEffective("qr_icon_color", effective.qr_icon_color)} onChange={(value) => patch("qr_icon_color", value)} />
        </ComposerSection>
      </div>
      ...
      <aside className="appearance-landing-preview-column">
        ...
        <div className="appearance-landing-core-compare">
          <button type="button" className="button button-secondary" disabled={!persistedRule || comparison.kind === "loading"} onClick={() => void compareSaved()}>
            {comparison.kind === "loading" ? t("compare.running") : t("compare.run")}
          </button>
          <small>{persistedRule ? t("compare.savedOnly") : t("compare.saveFirst")}</small>
